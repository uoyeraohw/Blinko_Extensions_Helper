...
- 当 `manifest.json` 中的版本号更新并推送到 `main` 分支时，会自动触发发布流程
- 自动打包扩展为 `blinko-extension.zip`
- 自动创建 GitHub Release 并上传文件
- Release tag 格式：`v{version}` (如 v1.2.0)

**发布新版本步骤：**
1. 更新 `manifest.json` 和 `package.json` 中的版本号
2. 更新 `CHANGELOG.md` 记录变更
3. 提交并推送：`git commit -m "chore: bump version to x.x.x" && git push origin main`
4. 等待 1-3 分钟，访问 Releases 页面查看新版本

**查看发布状态：**
- Actions 页面：查看工作流执行情况
- Releases 页面：下载发布的扩展包
